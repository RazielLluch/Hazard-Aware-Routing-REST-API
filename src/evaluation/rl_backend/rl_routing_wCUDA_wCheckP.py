...
Vendored snapshot — see ``rl_backend/README.md`` for upstream-merge policy.
The only local edit is the relative import of ``utils.graph_utils`` below
so the module resolves cleanly inside the ``rl_backend`` package.
"""
...
try:
    # Preferred: relative import when loaded as part of the rl_backend package
    # (i.e. via ``from src.evaluation.rl_backend import rl_routing_wCUDA_wCheckP``).
    from .utils.graph_utils import get_raw_osm_graph, to_training_graph
except ImportError:
    # Fallback: absolute import works when this file is run standalone with
    # the parent directory on sys.path (e.g. during retraining).
    from utils.graph_utils import get_raw_osm_graph, to_training_graph
